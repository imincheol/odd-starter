...
### Step 1. 수강 신청 (Order)
- **학생**: `tasks/roadmap.md`를 보고 할 일을 파악한 뒤, `tasks/_template/order.md`를 복사하여 오더 초안을 작성합니다.
- **교수**: 오더 내용이 스펙(`specs/`)과 일치하는지 검토하고 승인합니다. (**승인 전 코딩 금지**)

### Step 2. 과제 수행 (Progress)
- **학생**: 승인된 오더에 따라 `tasks/_template/progress.md`를 생성하고 작업을 시작합니다.
- **규칙**:
    1. 작업 시작 전, `order.md`의 `Context` 섹션을 참고하여 `progress.md`의 상단에 관련 문서를 링크하거나 숙지합니다.
